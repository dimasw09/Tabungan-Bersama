-- Tabungan Bersama Kakak & Mpip - Secure Auth Mode (Tahap 1)
-- Untuk project Supabase BARU, jalankan file ini sekali di SQL Editor.
-- Setelah itu buat dua user Auth dan jalankan supabase/link-users.example.sql.
...
values ('Mpip', 78000, 10, '#ffd6e7'), ('Kakak', 162000, 28, '#d8ecff')
on conflict (name) do update set monthly_amount = excluded.monthly_amount, payday = excluded.payday, color = excluded.color;

with month_seed as (
  select (date '2026-06-01' + (month_index || ' months')::interval)::date as month_date
  from generate_series(0, 23) as month_index
), default_members as (
  select id, monthly_amount, payday from public.members where lower(name) in ('mpip', 'kakak')
)
insert into public.monthly_deposits (member_id, year, month, due_date, required_amount, paid_amount, status)
select default_members.id,
       extract(year from month_seed.month_date)::int,
       extract(month from month_seed.month_date)::int,
       make_date(extract(year from month_seed.month_date)::int, extract(month from month_seed.month_date)::int,
         least(default_members.payday, extract(day from (date_trunc('month', month_seed.month_date)::date + interval '1 month - 1 day'))::int)),
       default_members.monthly_amount, 0, 'Belum Dibayar'
from month_seed cross join default_members
...
-- Pengaman tambahan: hanya anggota Kakak dan Mpip, nama tidak dapat diubah/dihapus.
create or replace function public.guard_fixed_members()
returns trigger
language plpgsql
as $$
begin
  if tg_op = 'INSERT' and lower(new.name) not in ('mpip', 'kakak') then raise exception 'Anggota dikunci hanya Kakak dan Mpip'; end if;
  if tg_op = 'UPDATE' then
    if lower(old.name) <> lower(new.name) then raise exception 'Nama anggota tidak boleh diubah'; end if;
    if lower(new.name) not in ('mpip', 'kakak') then raise exception 'Anggota dikunci hanya Kakak dan Mpip'; end if;
  end if;
  if tg_op = 'DELETE' then raise exception 'Anggota Kakak dan Mpip tidak boleh dihapus'; end if;
  return new;
end;
$$;

drop trigger if exists guard_fixed_members_trg on public.members;
create trigger guard_fixed_members_trg before insert or update or delete on public.members for each row execute function public.guard_fixed_members();
