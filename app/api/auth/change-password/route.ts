...
function validatePassword(password: string) {
  ...
  if (!/[A-Za-z]/.test(password) || !/[0-9]/.test(password)) return 'Password baru harus memiliki huruf dan angka.';
  return '';
}
...
export async function POST(request: NextRequest) {
  ...
  const ipLimit = await consumeRateLimit(admin, `change-password:ip:${getClientIp(request)}`, 20, 15 * 60);
  if (!ipLimit.allowed) {
    return noStoreJson(
  ...
  if (verifyError || !verifiedUser.user) return noStoreJson({ error: 'Session sudah tidak berlaku. Silakan masuk ulang.' }, 401);

  const userLimit = await consumeRateLimit(admin, `change-password:user:${verifiedUser.user.id}`, 5, 15 * 60);
  if (!userLimit.allowed) {
    return noStoreJson(
      { error: 'Terlalu banyak percobaan ganti password. Coba lagi beberapa saat.' },
      429,
  ...
  if (authUser.app_metadata?.must_change_password !== true) return noStoreJson({ error: 'Password akun ini sudah pernah diganti.' }, 409);
  ...
  const { error: samePasswordError } = await currentPasswordCheck.auth.signInWithPassword({ email: authUser.email, password: newPassword });
  if (!samePasswordError) return noStoreJson({ error: 'Password baru tidak boleh sama dengan password bawaan.' }, 400);
