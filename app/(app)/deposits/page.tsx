      ...
      <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm font-semibold text-slate-600">{row.proof_image_url ? 'Sudah tersedia' : 'Belum diunggah'}</p>
          </div>
        </div>
        {canManage ? (
          <div className="flex w-full items-center gap-2 sm:w-auto">
            <Button type="button" variant={remaining > 0 ? 'primary' : 'secondary'} className="min-h-11 flex-1 px-4 sm:min-w-40" onClick={() => remaining > 0 ? onQuick(row) : onCustom(row)} disabled={saving}>
              {remaining > 0 ? (hasPayment ? 'Lengkapi setoran' : 'Catat setoran') : 'Lihat & edit'}
            </Button>
            <details className="action-menu shrink-0">
              <summary aria-label={`Aksi lainnya untuk setoran ${row.member.name}`} className="flex h-11 w-11 items-center justify-center p-0"><AppIcon name="more" size={19} /></summary>
              <div className="action-menu-panel space-y-2">
                <Button type="button" variant="secondary" className="w-full whitespace-nowrap" onClick={() => onCustom(row)}>Edit detail</Button>
                {hasPayment || row.proof_image_url ? <Button type="button" variant="secondary" className="w-full whitespace-nowrap" onClick={() => onReset(row)}>Reset pembayaran</Button> : null}
                {row.deposit ? <Button type="button" variant="danger" className="w-full whitespace-nowrap" onClick={() => onDelete(row)}>Hapus periode</Button> : null}
              </div>
            </details>
          </div>
        ) : (
          <p className="w-full rounded-2xl bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-500 sm:max-w-xs">
            Mode lihat saja. Hanya {row.member.name} yang dapat mengubah setoran ini.
          </p>
        )}
      </div>
    </div>
...
function getPaymentDraftWarnings(draft: PaymentDraft) {
  ...
  if (draft.actual_transfer_date && isValidDateText(draft.actual_transfer_date)) {
    ...
    if (transferYear !== draft.year || transferMonth !== draft.month) {
      warnings.push(`Tanggal transfer berada di luar periode ${monthLabel(draft.year, draft.month)}. Kalau lanjut, data tetap dicatat ke bulan yang dipilih.`);
    }
  }
...
export default function DepositsPage() {
  ...
  async function uploadProof(file: File, depositId: string) {
    ...
    const path = `${householdId}/${depositId}/${Date.now()}.${ext}`;
  ...
  async function persistPaymentDraft(draft: PaymentDraft) {
    ...
    try {
      ...
      if (uploadedPath && oldPath && oldPath !== uploadedPath && !oldPath.startsWith('http')) {
        const { error: cleanupError } = await supabase.storage.from('transfer-proofs').remove([oldPath]);
      ...
      if (uploadedPath) await supabase.storage.from('transfer-proofs').remove([uploadedPath]);
    ...
    setConfirmAction({
      ...
      onConfirm: async () => {
        ...
        if (oldProofPath && !oldProofPath.startsWith('http')) {
          const { error: storageError } = await supabase.storage.from('transfer-proofs').remove([oldProofPath]);
